...
function remove_vm(){
	...
	then
		...
		sudo -u $(logname) sed -i -e '/echo $'${rmvmname}'_FDA/c\' ${CONFIG_LOC} > /dev/null 2>&1
		sudsudo -u $(logname) sed -i -e '/echo $'${rmvmname}'_RAM/c\' ${CONFIG_LOC} > /dev/null 2>&1o -u $(logname) sed -i -e 'N;/^\n$/D;P;D;' ${CONFIG_LOC} > /dev/null 2>&1
